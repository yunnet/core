...
func (r *Stream) Close() {
	...
	engine.Print(Yellow("Stream destoryed :"), BrightCyan(r.StreamPath))
...
func (r *Stream) Publish() bool {
	...
	engine.Print(Green("Stream publish:"), BrightCyan(r.StreamPath))
	r.timeout = time.AfterFunc(config.PublishTimeout, func() {
		engine.Print(Yellow("Stream timeout:"), BrightCyan(r.StreamPath))
		r.Close()
	})
	//触发钩子
...
func (r *Stream) Subscribe(s *Subscriber) {
	if s.Stream = r; r.Err() == nil {
		...
		engine.Print(Sprintf(Yellow("subscribe :%s %s,to Stream %s"), Blue(s.Type), Cyan(s.ID), BrightCyan(r.StreamPath)))
		...
		engine.Print(Sprintf(Yellow("%s subscriber %s added remains:%d"), BrightCyan(r.StreamPath), Cyan(s.ID), Blue(len(r.Subscribers))))
	}
}
...
func (r *Stream) UnSubscribe(s *Subscriber) {
	if r.Err() == nil {
		...
		if deleted {
			engine.Print(Sprintf(Yellow("%s subscriber %s removed remains:%d"), BrightCyan(r.StreamPath), Cyan(s.ID), Blue(len(r.Subscribers))))
			l := len(r.Subscribers)
