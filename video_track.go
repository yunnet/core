...
func (vt *VideoTrack) pushNalu(ts uint32, cts uint32, nalus ...[]byte) {
	...
	vt.writeByteStream = func() {
		...
		engine.BigEndian.PutUint24(tmp, vt.CompositionTime)
		vt.Buffer.Write(tmp[:3])
		for _, nalu := range vt.NALUs {
			engine.BigEndian.PutUint32(tmp, uint32(len(nalu)))
			vt.Buffer.Write(tmp)
			...
			vt.PushNalu = func(ts uint32, cts uint32, nalus ...[]byte) {
				...
				vt.PushNalu = func(ts uint32, cts uint32, nalus ...[]byte) {
					...
					for _, nalu := range nalus {
						...
						default:
							engine.Printf("%s,nalType not support yet:%d,[0]=0x%X", vt.Stream.StreamPath, naluType, nalu[0])
						}
					}
					if nonIDRs > 0 {
						vt.push()
...
func (vt *VideoTrack) PushByteStream(ts uint32, payload []byte) {
	if payload[1] == 0 {
		...
		vt.setTS(ts)
		vt.Payload = payload
		vt.CompositionTime = engine.BigEndian.Uint24(payload[2:])
		vt.ResetNALUs()
